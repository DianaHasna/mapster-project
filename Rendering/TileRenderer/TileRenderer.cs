...
public static class TileRenderer
{
    public static BaseShape Tessellate(this MapFeatureData feature, ref BoundingBox boundingBox, ref PriorityQueue<BaseShape, int> shapes)
    {
        ...
        if (!feature.Properties.Any(p => p.Key == MapElement.highway && MapFeature.HighwayTypes.Any(v => p.Value.StartsWith(v))))
        {
            if (!feature.Properties.Any(p => p.Key == MapElement.water) || feature.Type == GeometryType.Point)
            {
                if (!Border.ShouldBeBorder(feature))
                {
                    if (!PopulatedPlace.ShouldBePopulatedPlace(feature))
                    {
                        if (!feature.Properties.Any(p => p.Key == MapElement.railway))
                        {
                            if (!feature.Properties.Any(p => p.Key == MapElement.natural && featureType == GeometryType.Polygon))
                            {
                                if (!feature.Properties.Any(p => p.Key == MapElement.boundary && p.Value.StartsWith("forest")))
                                {
                                    if (!feature.Properties.Any(p => p.Key == MapElement.landuse && (p.Value.StartsWith("forest") || p.Value.StartsWith("orchard"))))
                                    {
                                        if (feature.Type != GeometryType.Polygon || !feature.Properties.Any(p
                                                                                     => p.Key == MapElement.landuse && (p.Value.StartsWith("residential") || p.Value.StartsWith("cemetery") || p.Value.StartsWith("industrial") || p.Value.StartsWith("commercial") ||
                                        ...
                                        {
                                            if (feature.Type != GeometryType.Polygon || !feature.Properties.Any(p
                                                                                             => p.Key == MapElement.landuse && (p.Value.StartsWith("farm") || p.Value.StartsWith("meadow") || p.Value.StartsWith("grass") || p.Value.StartsWith("greenfield") ||
...
feature.Properties.Any(p => p.Key == MapElement.landuse && (p.Value.StartsWith("reservoir") || p.Value.StartsWith("basin"))))
                                                {
                                                ...
                                                {
                                                    if (feature.Type != GeometryType.Polygon || !feature.Properties.Any(p => p.Key == MapElement.building))
                                                    {
                                                        if (feature.Type != GeometryType.Polygon || !feature.Properties.Any(p => p.Key == MapElement.leisure))
                                                        {
                                                            if (feature.Type != GeometryType.Polygon || !feature.Properties.Any(p => p.Key == MapElement.amenity))
                                                            {
                                                            }
                                                            else
                                                            {
