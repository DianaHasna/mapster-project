...
public readonly ref struct MapFeatureData
{
    ...
    public Dictionary<MapElement, string> Properties { get; init; }
}
...
public unsafe class DataFile : IDisposable
{
    ...
    private static string[] mapElementList =
    {
        "highway",
        "water",
        "railway",
    ...
    public static string[] MapElementList { get => mapElementList; set => mapElementList = value; }
    ...
    public void ForeachFeature(BoundingBox b, MapFeatureDelegate? action)
    {
        ...
        for (var i = 0; i < tiles.Length; ++i)
        {
            ...
            for (var j = 0; j < header.Tile.Value.FeaturesCount; ++j)
            {
                ...
                if (isFeatureInBBox)
                {
                    var properties = new Dictionary<MapElement, string>(feature->PropertyCount);
                    for (var p = 0; p < feature->PropertyCount; ++p)
                    {
                        ...
                        try
                        {
                            MapElement geoTypeKey = (MapElement)Enum.Parse(typeof(MapElement), key.ToString());
